...
assert.equal(rough.outcome.counts.unresolved,1);
assert.deepEqual(rough.outcome.rows.filter(r=>r.kind==='unresolved').map(r=>r.original),['#C5C263']);
...
assert.equal(solid.outcome.counts.unresolved,0);
assert(solid.outcome.counts.substituted>0);
// A suggestion must expose the same recipe and missing colours as its export.
const blendReels=['#0080C0','#FFFFFF','#000000','#FF0000'].map(color=>({color,type:'PLA'}));
const blended=recommendPalette({sources:{1:'#0080C0',2:'#FFFFFF',3:'#000000',4:'#FF0000',5:'#7B2E4D'},loaded:blendReels,locked:[true,true,true,true]});
assert.equal(blended.outcome.counts.unresolved,0);
assert.equal(blended.outcome.blendCount,1);
assert.equal(blended.outcome.counts.preserved,4);
const recipe=blended.outcome.rows.find(r=>r.kind==='blended').recipe;
assert.deepEqual([recipe.a,recipe.b,recipe.percent],[1,4,50]);
