...
assert(!metadata.includes('key="support_speed"'),'no crash-inducing object support-speed override');
const cfg=JSON.parse(new TextDecoder().decode(out.entries.get('Metadata/project_settings.config')));
assert.equal(cfg.support_speed,'150','designer speed retained globally');
assert(metadata.includes('key="brim_type" value="auto_brim"'),'designer brim retained');
